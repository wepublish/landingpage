        ...
        {hasSteps && (
          <>
            ...
            <div className="flex justify-between items-center pt-4 gap-3">
              ...
              {isFirstStep && (
                <button className="w-full px-6 py-3 text-gray-900 font-semibold rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer flex items-center justify-center gap-2" style={{ backgroundColor: formConfig.buttonColor ?? '#FFD60A', color: formConfig.buttonFontColor ?? '#000000' }} type="submit" disabled={isSubmitting}>
                  Jetzt kostenlos abonnieren {isSubmitting && <Image src={spinner} alt="" width={20} height={20} />}
                </button>
              )}

              {isMiddleStep && (
                <button className="ml-auto px-6 py-2.5 text-gray-900 font-medium rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer flex items-center justify-center gap-2" style={{ backgroundColor: formConfig.buttonColor ?? '#FFD60A', color: formConfig.buttonFontColor ?? '#000000' }} type="submit" disabled={isSubmitting}>
